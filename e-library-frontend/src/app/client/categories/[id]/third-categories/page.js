...
function ThirdCategories({ params }) {
    ...
    const fetchCategories = async () => {
      ...
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_URL}/categories/child-categories/${cat_id}?type=3`,
          {
            headers: {
      ...
      <div className="w-full md:w-1/2 md:mx-auto grid grid-cols-2 gap-2 mt-2">
        ...
        {!loading &&
          categories.map((category, index) => (
            ...
            >
              <Link
                href={`/client/categories/${category.id}/third-category-books`}
...
export default WithAuth(ThirdCategories);
