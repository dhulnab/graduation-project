...
function SecondCategories({ params }) {
    ...
    const fetchCategories = async () => {
      ...
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_URL}/categories/child-categories/${cat_id}?type=2`,
          {
            headers: {
      ...
      <div className="w-full md:w-1/2 md:mx-auto grid grid-cols-2 gap-2 mt-2">
        ...
        {!loading &&
          categories.map((category, index) => (
            ...
            >
              <Link
                href={`/client/categories/${category.id}/second-category-books`}
              ...
              <div className="w-full flex items-center justify-center text-[10px] p-1 h-1/5 text-gray-600 border-t">
                <div className="flex items-center">
                  <Link
                    href={`/client/categories/${category.id}/third-categories`}
                    className="underline"
                  >
                    {"Subcategories"}
                  </Link>
...
export default WithAuth(SecondCategories);
